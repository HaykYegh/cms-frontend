...
interface IIndexState {
    ...
    validation: {
        newUser: {
            ...
            phoneNumberFB: IVALIDATION,
            email: IVALIDATION,
            country: IVALIDATION,
            password: IVALIDATION,
            confirmPassword: IVALIDATION,
            passwordValidRules: any;
        }
    },
    popup: {
        isShown: boolean,
...
class Index extends React.Component<IIndexProps, IIndexState> {
        ...
        this.state = {
            ...
            popup: {
                ...
                newUser: {
                    isValidNumber: false,
                    isValidEmail: false,
                    phoneNumber: FOR_BUSSINESS[PREFIX] ? `${FOR_BUSSINESS[PREFIX].phoneCode}${FOR_BUSSINESS[PREFIX].fourNumber}` : "",
                    email: "",
                    country: null,
                    password: "",
                    confirmPassword: "",
    ...
    initRequests: any = (state: IIndexState, offset: number = 0, isSearch: boolean = false, isReset: boolean = false, isPaging: boolean = false): void => {
        const {
            limit, initialFilters: {registration, phone, country, platform, userGroup, channel},
    ...
    handlePhoneOrEmailChange = ({currentTarget: {value}}: React.ChangeEvent<HTMLInputElement>): void => {
        const newState: IIndexState = {...this.state};
        newState.initialFilters.phone.value = value;
        newState.request.search.isDisabled = false;
        newState.request.search.isChanged = true;
        this.setState(newState);
    };
    ...
    handleModalCloseFB = (): void => {
        const {validation} = this.state;
        const newState: IIndexState = {...this.state};

        newState.popup.isShown = false;
        newState.popup.newUser = {
            phoneNumber: `${FOR_BUSSINESS[PREFIX].phoneCode}${FOR_BUSSINESS[PREFIX].fourNumber}`,
            email: "",
            country: null,
            password: "",
            confirmPassword: "",
    ...
    handleNewUserPhoneNumberChangeFB = ({currentTarget: {value}}: React.ChangeEvent<HTMLInputElement>) => {
        const newState: IIndexState = {...this.state};
        const VALID_LENGTH: number = 12;
        const phoneCode: string = `${FOR_BUSSINESS[PREFIX].phoneCode}${FOR_BUSSINESS[PREFIX].fourNumber}`;
        const isValidStartCharacters: boolean = new RegExp(`^${phoneCode}`).test(value);
        const isValidAllCharacters: boolean = new RegExp(`^${phoneCode}(.{${VALID_LENGTH - phoneCode.length}})?$`).test(value);

        newState.popup.newUser.phoneNumber = value;
        newState.validation.newUser.phoneNumber.value = (
            (value === "") ? null : (isValidStartCharacters && isValidAllCharacters) ? "success" : "error"
        );
        newState.validation.newUser.phoneNumberFB.message = isValidStartCharacters ? "" : `Phone number need to start with ${phoneCode},`;
        newState.validation.newUser.phoneNumberFB.message += isValidAllCharacters ? "" : `Phone number need to have ${VALID_LENGTH} characters.`;
        newState.popup.newUser.isValidNumber = isValidAllCharacters;
    ...
    handleNewUserPasswordChange = ({currentTarget: {value, name}}: React.ChangeEvent<HTMLInputElement>): void => {
        ...
        if (name === "password") {
            ...
            newState.popup.newUser.passwordValidRules.atLeastOneLowerCase = true;
            newState.popup.newUser.passwordValidRules.atLeastOneUpperCase = true;
            newState.popup.newUser.passwordValidRules.atLeastOneNumeric = true;
            newState.popup.newUser.passwordValidRules.atLeastOneSpecialCharacter = true;
            newState.popup.newUser.passwordValidRules.eightCharactersOrLonger = true;
            newState.validation.newUser.password.value = value.length >= PASSWORD_MIN_LENGTH ? "success" : value === "" ? null : "error";
            newState.validation.newUser.password.message = (value.length >= PASSWORD_MIN_LENGTH || value === "") ? ""
    ...
    handleNewUserToggleDisabled = (state: any): void => {
        const {selectedOption, country, password, confirmPassword, isValidNumber, isValidEmail}: any = {...state.popup.newUser};
    ...
    handleCreateUser = (e: React.MouseEvent<HTMLButtonElement>): void => {
        ...
        if (newUser.selectedOption === NEW_USER_CREATE.TYPE.VIA_PHONE_NUMBER) {
            ...
            newUserData.phoneCode = newUser.country.phone_code;
        }
        ...
        if (!atLeastOneLowerCase.test(newUserData.password)) {
            newState.popup.newUser.passwordValidRules.atLeastOneLowerCase = false
        }

        if (!atLeastOneUpperCase.test(newUserData.password)) {
            newState.popup.newUser.passwordValidRules.atLeastOneUpperCase = false
        }

        if (!atLeastOneNumeric.test(newUserData.password)) {
            newState.popup.newUser.passwordValidRules.atLeastOneNumeric = false
        }

        if (!atLeastOneSpecialCharacter.test(newUserData.password)) {
            newState.popup.newUser.passwordValidRules.atLeastOneSpecialCharacter = false
        }

        if (!eightCharactersOrLonger.test(newUserData.password)) {
            newState.popup.newUser.passwordValidRules.eightCharactersOrLonger = false
        }

        const bool: boolean = Object.keys(newState.popup.newUser.passwordValidRules).every(item => newState.popup.newUser.passwordValidRules[item])
        if (!bool) {
    ...
    render(): JSX.Element {
        const {
            ...
            additionalFilters: {activity, network, blocked, balance, callCount, duration, messageCount}, validation, popup: {isShown, newUser}, nickname
        ...
        return (
            <div>
                    ...
                    <div className="container-fluid">
                        ...
                        <div className="row">
                            ...
                            <div className="col-lg-3 col-md-6 col-sm-12 col-xs-12">
                                <FormGroup>
                                    <ControlLabel htmlFor="phone-number">Phone number / Email</ControlLabel>
                                    {/*<AsyncSelect*/}
                                    ...
                                    <FormControl
                                      ...
                                      value={initialFilters.phone.value}
                                      placeholder="Phone number / Email"
                            ...
                            <div className="col-lg-3 col-md-6 col-sm-12 col-xs-12">
                                <FormGroup>
                                    <ControlLabel htmlFor="channel">Channel Users by Date</ControlLabel>
                                    <AsyncSelect
                                        inputId="channel-name"
                                        name="channel-name"
                                        placeholder="Channel name"
                        ...
                        <div className="row">
                            ...
                            <div className="col-lg-6 col-md-6 col-sm-6 col-xs-8 text-right padder-t-16">
                                {process.env.APP_PREFIX !== "ig" && <DropdownButton title="Add filter" id="add-filter">{addFilter}</DropdownButton>}
                                <button
                            ...
                            {users.records && users.records.length > 0 && users.records.map((user, index) => {
                                ...
                                return (
                                    <tr key={user.userId} className="cursor-pointer" onClick={userView}>
                                        <td>{N}</td>
                                        <td>{user.email ? user.email : user.number}</td>
                ...
                {/*Create user*/}
                {!FOR_BUSSINESS[PREFIX] ? <Modal show={isShown} onHide={this.handleModalClose}>
                             ...
                             <Modal.Body>
                                <div className="container-fluid">
                                    <div className="row">
                                        ...
                                        <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                            <FormGroup
                                                // validationState={validation.newUser.password.value}
                                            >
                                                ...
                                                {newUser.passwordValidRules.atLeastOneLowerCase === false &&
                                                <HelpBlock style={{color: "#f05050"}}>Password need to have at least one
                                                    lowercase.</HelpBlock>}
                                                {newUser.passwordValidRules.atLeastOneUpperCase === false &&
                                                <HelpBlock style={{color: "#f05050"}}>Password need to have at least one
                                                    uppercase.</HelpBlock>}
                                                {newUser.passwordValidRules.atLeastOneNumeric === false &&
                                                <HelpBlock style={{color: "#f05050"}}>Password need to have at least one numeric
                                                    character.</HelpBlock>}
                                                {newUser.passwordValidRules.atLeastOneSpecialCharacter === false &&
                                                <HelpBlock style={{color: "#f05050"}}>Password need to have at least one special
                                                    character.</HelpBlock>}
                                                {newUser.passwordValidRules.eightCharactersOrLonger === false &&
                                                <HelpBlock style={{color: "#f05050"}}>Password need to have eight characters or
                                                    longer.</HelpBlock>}
                                            </FormGroup>
                                        </div>
                                        <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                            ...
                                            >Create user &nbsp;{createUser.isProcessing &&
                    ...
                    <Modal show={isShown} onHide={this.handleModalCloseFB}>
                        <Modal.Header closeButton={true}>
                            <span className="font-semi-bold text-lg">Create New User</span>
                        </Modal.Header>
                        <Modal.Body>
                                        ...
                                        </label>
                                        {
                                            ...
                                            <FormGroup validationState={validation.newUser.phoneNumber.value}>
                                                <FormControl
                                                    ...
                                                    autoFocus={true}
                                                    value={newUser.phoneNumber}
                                                    onChange={this.handleNewUserPhoneNumberChangeFB}
                                                    autoComplete={"new-password"}
                                                />
                                                {validation.newUser.phoneNumberFB.message.split(",").map((message) => (
