...
import {checkUsersCount, notificationSend} from "ajaxRequests/notification";
...
class CountriesAndPlatforms extends React.Component<ICountriesAndPlatformsProps, ICountriesAndPlatformsState> {
    ...
    handleCheckUsersCount = (event: React.MouseEvent<HTMLButtonElement>): void => {
        ...
        checkUsersCount(countryIDs, platformIDs, startsWith).then(({data}: AxiosResponse) => {
            if (data.err) {
    ...
    handleNotify = (e: React.MouseEvent<HTMLButtonElement>): void => {
        ...
        notificationSend(countryIDs, platformIDs, startsWith, senderId, {message}).then(({data}: AxiosResponse) => {
            if (data.err) {
                throw new Error(JSON.stringify(data));
            }
