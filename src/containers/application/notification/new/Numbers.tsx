...
import {validateNumber, isNumeric, createOption, testableNumbersValidate, validateZangiNumber} from "helpers/DataHelper";
...
class Numbers extends React.Component<INumbersProps, INumbersState> {
    ...
    handleFromNumberInputChange = (inputValue: string): any => {
        ...
        const isValidPhoneNumber: boolean = validateNumber(valueForValidate)?.isValid || false;
        const isValidZangiNumber: boolean = validateZangiNumber(valueForValidate.slice(1));
        const isValid: boolean = isValidPhoneNumber || isValidZangiNumber;
    ...
    handleNotify = (event: React.MouseEvent<HTMLButtonElement>): void => {
        ...
        const newState: INumbersState = this.state;
        const {notification: {message}, phone: {fromNumbers}, emails: {emailsList}, validation, selectedSender} = this.state;
    ...
    render(): JSX.Element {
        ...
        const isReady: boolean = (message !== "" && (fromNumbers.value.length !== 0 || emailsList.value.length !== 0) && selectedSender !== null);
        return (
